...
Generate `knowz-vaults.md` using the bundled `vault-routing-template.md` beside this setup flow — including its **Trust & Freshness** block, so every consumer treats vault knowledge as verify-first (point-in-time, may be stale, check against live code/docs). Write the file to the project root.
