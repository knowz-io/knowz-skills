...
All prompt files live in `.github/prompts/` and are generated by `/knowzcode:setup` (or `npx @knowzai/knowzcode`). They use YAML frontmatter with `agent: "agent"` for file editing capability.
