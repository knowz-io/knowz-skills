...
- **Ask-once profile choice (low-intrusion cost consent):** the profile is chosen exactly once per project and persisted. `/knowzcode:init` now asks "Frontier (recommended) or Teams?" during setup; if no `profile:` line exists, the first `/knowzcode:work` asks the same single question and writes the answer to `knowzcode_orchestration.md` — no run ever re-asks. Bypasses: an explicit `--profile` flag, an existing `profile:` line (including pre-0.19 configs with `profile: teams`, which keep their prior behavior), and autonomous mode (proceeds on `frontier` with an `[AUTO-DEFAULT]` log line instead of blocking). `/audit`, `/explore`, and `/fix` never prompt.
