...
- `--profile frontier` runs the analyst/architect/reviewer research on Fable 5 (falls back to Opus if Fable is unavailable); knowledge-liaison stays on Sonnet; other profiles are unchanged in explore (research-only, no advisor tool)
