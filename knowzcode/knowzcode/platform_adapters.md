...
Any agent can send ad-hoc knowledge to the scribe at any time. The scribe handles routing, dedup, and formatting. If MCP is unavailable, the scribe queues captures to `knowzcode/pending_captures.md` for later sync. When no scribe is active, use `/knowz save "insight"` for automatic routing.
...
.agents/agents/knowzcode-knowz-scout.md
.agents/agents/knowzcode-knowz-scribe.md
...
prompt = """Read .gemini/skills/knowzcode-plan/SKILL.md for full instructions.
...
| Research | `#prompt:knowzcode-plan` | Investigate before implementing |
